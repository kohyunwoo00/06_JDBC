...
public class JDBCExample2 {
	public static void main(String[] args) {
		// EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여를
		// 급여 오름차순으로 조회
		/* 1. JDBC 객체 참조 변수 선언 */
		...
		try {
			...
			/*5. Statement 객체를 이용해서 SQL을 DB로 전달 후 수행
			    ...
			    2) DML문    : excuteUdapte() -> 결과 행의 개수(int)*/
			...
			while(rs.next()) {
				// rs.next() : ResultSet의 Cursor를 다음행으로 이동
				// 다음 행이 있으면 T, 없으면 F
			...
			/* 7. 사용 완료된 JDBC 객체 자원 반환 -> finally에서 */
			try {

...
public class JDBCExample1 {
	public static void main(String[] args) {
		 ...
		 * - 다룰 때 CUSOR를 이용해 1행 씩 접근
		 * 
		 * 4) DriverManager 클래스
		 * - DB 연결 정보와 JDBC 드라이버를 이용해서 원하는 DB와 연결할 수 있는 
		...
		try {
			...
			// --> Connection 객체를 만들 때 JVM이 자동으로 연결됨
			...
			conn = DriverManager.getConnection(type + host + port + dbName, // DB URL
					userName,  // 사용자계정명
					password  // 비밀번호
					); 
			 ...
			 * 
			 * !! 주의 사항 !!
			 * - JDBC 코드에서 SQL 잓성 시 마지막에 세미콜론 작성 X
			 * 	-> 작성 시 : "SQL 명령어가 올바르게 종료되지 않았습니다." 예외 발생
			 * (왜? JDBC는 한번에 SQL을 1개 씩만 실행
			 * ==> DB에 SQL 전달 시 자동으로 끝에; 붙어줌)
			 * */
			...
			// ResultSet.stmt.executeQuery(sql) - SELECT문이 작성된 sql을 DB로 전달해 수행 후
			// 													DB에서 반환된 결과를 ResultSet 형태로 변환 후 return
			
			/* 6. 조회 결과가 저장된 ResultSet을 1행 씩 접근하여 각 행에 기록된 컬럼 값 얻어오기 
			 * 1행씩 접근하여 각 행에 기록된 컬럼 값 얻어오기
			 * - 1행씩 접근 할때 자동으로 Cursor를 이용함
			 * */
			
			// rs.next() : Cursor를 다음 행으로 이동 후 행이 있으면 T, 없으면 F 반환
			//             단, 첫 호출 시에는 1행으로 이동
			while(rs.next()) {
				// rs.get자료형(컬럼명 || 순서) -> 현재 Cursor가 가리키는 행의 컬럼 값을 얻어옴
				// - 자료형 자리에는 DB 값을 읽어 왔을  때 Java에서 저장하기 적절한 자료형을 작성

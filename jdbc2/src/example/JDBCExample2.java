...
public class JDBCExample2 {
	public static void main(String[] args) {
		...
		/* java.sql.PreparedStatement : 분비된 Statement
		 ...
		 * 
		 * PreparedStatement는 ?를 이용해서 파라미터(값)만 연결하기 때문에
		 * SQL Injection을 방지 할 수 있다.
		 */
		...
		try {
			...
			// -> 객체 생성과 동시 SQL을 매개 변수로 전달해서 담아둠 -> SQL 파악하기 위해서
			pstmt = conn.prepareStatement(sql);
			
			/* ?(placeholder)에 알맞은 값 세팅 */
			// pstmt.set자료형(?순서, 대입할값);
			
			pstmt.setString(1, id); // id 변수값에 '' 추가하여 sql에 대입
